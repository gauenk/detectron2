...
RPN_HEAD_FLOWS_REGISTRY = Registry("RPN_HEAD_FLOWS")
RPN_HEAD_FLOWS_REGISTRY.__doc__ = """
...
def build_rpn_head_flows(cfg, input_shape):
    """
    Build an RPN head defined by `cfg.MODEL.RPN.HEAD_NAME`.
    """
    name = cfg.MODEL.RPN.HEAD_NAME
    return RPN_HEAD_FLOWS_REGISTRY.get(name)(cfg, input_shape)


@RPN_HEAD_FLOWS_REGISTRY.register()
class StandardRPNHeadFlows(nn.Module):
    """
    ...
    def __init__(
        ...
        self.objectness_logits = nn.Conv2d(cur_channels, num_anchors,
                                           kernel_size=1, stride=1)
        # 1x1 conv for predicting box2box transform deltas
        self.anchor_deltas = nn.Conv2d(cur_channels, num_anchors * box_dim,
                                       kernel_size=1, stride=1)
    ...
    def forward(self, features: List[torch.Tensor]):
        ...
        for x in features:
            ...
            t = self.conv(x)
            # t = rearrange(t,'(b t) c h w -> b t c h w',b=B)
            # print("t.shape: ",t.shape)
            # shape_str = '(b t) c h w -> b t c h w'
            # pred_logit_x = rearrange(self.objectness_logits(t),shape_str,b=B)
            # pred_objectness_logits.append(pred_logit_x)
            # pred_anchor_x = rearrange(self.anchor_deltas(t),shape_str,b=B)
            # pred_anchor_deltas.append(pred_anchor_x)
...
class RPNFlows(nn.Module):
    ...
    def forward(
        ...
        pred_anchor_deltas = [
            ...
            x.view(x.shape[0], -1, self.anchor_generator.box_dim,
                   x.shape[-2], x.shape[-1])
            .permute(0, 3, 4, 1, 2)
            .flatten(1, -2)
